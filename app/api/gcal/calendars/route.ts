...
import {
  getCalendarsFromGoogle,
  saveAuthorizedCalendars,
  ...
  try {
    ...
    if (!Array.isArray(calendars) || calendars.length === 0) {
      return NextResponse.json(
        {
          success: false,
          error: "Payload must contain a non-empty array of calendars",
        },
        { status: 400 },
      );
    }

    const schemaValid = calendars.every(
      (cal) => cal?.id && cal?.etag && cal?.summary && cal?.timeZone,
    );
