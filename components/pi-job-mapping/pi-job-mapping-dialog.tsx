...
export function MappingDialog({ 
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (jobWrapperRef.current && !jobWrapperRef.current.contains(event.target as Node)) {
        setJobDropdownOpen(false);
      }
      if (piWrapperRef.current && !piWrapperRef.current.contains(event.target as Node)) {
