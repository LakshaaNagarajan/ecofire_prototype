...
export function QBOCircles({ onSelectJob }: QBOCircleProps) {
    ...
    const fetchQbosAndMappings = async () => {
      try {
        ...
        const jobMappingsPromises = qbosData.map(async (qbo: QBOs) => {
          const response = await fetch(`/api/qbo-job-mappings?qboId=${qbo._id}`);
  ...
  if (loading) {
    return <div className="flex justify-center p-4">Loading QBO mappings...</div>;
  }

  if (qbos.length === 0) {
    return <div className="text-center p-4 text-muted-foreground">No QBOs found.</div>;
  }
  ...
  const getQboColor = (qboName: string) => {
    ...
    const hash = qboName.split('').reduce((acc, char) => acc + char.charCodeAt(0), 0);
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  ...
                  >
                    ...
                    <div className="text-xs opacity-80">{qbo.targetValue} {qbo.unit}</div>
                ...
                <TooltipContent>
                  ...
                  <p className="text-xs">Target: {qbo.targetValue} {qbo.unit}</p>
                </TooltipContent>
              </Tooltip>
            </TooltipProvider>
          );
