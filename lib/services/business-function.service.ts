...
import DefaultsTracker from "../models/business-functions-defaults-tracker.model";
...
export class BusinessFunctionService {
  ...
  async checkNameExists(name: string, userId: string): Promise<boolean> {
    try {
      await dbConnect();
      const found = await BusinessFunction.findOne({ name, userId })
  ...
  async getAllBusinessFunctions(
    ...
    try {
      ...
      const existingFunctions = await BusinessFunction.countDocuments({
        userId,
      });
  ...
  async deleteBusinessFunction(id: string, userId: string): Promise<boolean> {
    try {
      await dbConnect();
      const businessFunction = await BusinessFunction.findOne({
        _id: id,
        userId,
      });
