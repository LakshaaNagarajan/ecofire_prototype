...
import { PIJobMappingService } from './pi-job-mapping.service';
...
export class JobDuplicationService {
  ...
  private mappingService: PIJobMappingService;

  constructor() {
    // Initialize service instances
    this.jobService = new JobService();
    this.taskService = new TaskService();
    this.mappingService = new PIJobMappingService();
  }
   ...
   */
  async duplicateJob(sourceJobId: string, newJobData: Partial<Jobs>, userId: string): Promise<Jobs> {
    // Get the source job
    ...
    const tasksResult = await this.taskService.getTasksByJobId(sourceJobId, userId);
    ...
    try {
      ...
      const originalJobMappings = await this.mappingService.getMappingsByJobId(sourceJobId);
