...
export async function updateJobImpactValues(userId: string) {
  try {
    ...
    PIQBOmappings.forEach(mapping => {
      ...
      const piPointsValues: [number, number] = PIPointsMap.get(piId);
      const impactValue = mapping.qboImpact || 0;
      const piPoints = impactValue * QBOPointsMap.get(qboId);
    ...
    JobPImappings.forEach(mapping => {
      ...
      // Add to the existing value or initialize
      jobImpactMap.set(jobId, (jobImpactMap.get(jobId) || 0) + impactValue * PIPointsMap.get(piId)[0] / PIPointsMap.get(piId)[1]);
    });
