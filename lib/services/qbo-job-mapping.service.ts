...
export class QBOJobMappingService {
  ...
  async getJobsMappedToQBO(qboId: string, userId: string): Promise<Jobs[]> {
    try {
      ...
      const piQboMappings = await this.piQboMappingService.getMappingsForQBO(qboId, userId);
      ...
      const jobPiMappings = await Promise.all(
        piIds.map(piId => this.piJobMappingService.getAllMappingJP(userId)
          .then(mappings => mappings.filter(mapping => mapping.piId === piId))
        )
      );
      ...
      const jobIds = [...new Set(allJobPiMappings.map(mapping => mapping.jobId))];
      ...
      return jobs.filter(job => job !== null) as Jobs[];
    } catch (error) {
  ...
  async getJobsWithMappingDetails(qboId: string, userId: string): Promise<any[]> {
    try {
      await dbConnect();
      ...
      const allJobPiMappings = await this.piJobMappingService.getAllMappingJP(userId);
      
      // Enhance job data with mapping details
      return jobs.map(job => {
        // Find all PI mappings for this job
        const jobPiMappings = allJobPiMappings.filter(mapping => mapping.jobId === job._id);
        
        // Calculate the impact path from job through PIs to QBO
        const impactPaths = jobPiMappings.map(jobPiMapping => {
          const relatedPiQboMapping = piQboMappings.find(mapping => mapping.piId === jobPiMapping.piId);
          ...
          if (relatedPiQboMapping) {
            return {
              ...
              jobContribution: (jobPiMapping.piImpactValue / (jobPiMapping.piTarget || 1)) * relatedPiQboMapping.qboImpact
            };
          }
          return null;
        ...
        return {
          ...job,
          impactPaths,
          totalQBOContribution: impactPaths.reduce((sum, path) => sum + (path?.jobContribution || 0), 0)
        };
      });
    } catch (error) {
